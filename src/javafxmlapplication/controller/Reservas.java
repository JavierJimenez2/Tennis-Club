...
//        when date is changed, the court slots are updated
    ...
    public void deleteAction(ActionEvent actionEvent) {
        if(!listView.getSelectionModel().getSelectedItem().isOlderForDay(LocalDate.now())){   //checks if booking was of a previous day
        ...
        } else if(Duration.between(listView.getSelectionModel().getSelectedItem().getBookingDate(), LocalDateTime.now()).compareTo(Duration.ofHours(24)) < 0){
            Alert alert = new Alert(Alert.AlertType.WARNING);
            ...
            alert.showAndWait().ifPresent(buttonType -> {
                if(buttonType == buttonTypeYes) {
                    ...
                    JavaFXMLApplication.dialogBox("info","Action Canceled","Reservation deletion canceled.");
                }
            });
        } else {
            showConfirmationWindow();
        ...
        alert.showAndWait().ifPresent(buttonType -> {
            if(buttonType == buttonTypeYes) {
                ...
                try {
                    ...
                    JavaFXMLApplication.dialogBox("success","Process completed successfully","Your booking has been removed.");
